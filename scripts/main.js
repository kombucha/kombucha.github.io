!(function(global) {
  ...
  (Sequence = function(str, next, fail, done) {
    ...
    (Sequence.prototype.keydown = function(keyCode) {
      var i = this.idx;
      return keyCode !== this.keys[i]
        ? void (
            i > 0 && (this.reset(), this.fail(this.str), cheet.__fail(this.str))
          )
        : (this.next(this.str, this.seq[i], i, this.seq),
          cheet.__next(this.str, this.seq[i], i, this.seq),
          void (
            ++this.idx === this.keys.length &&
            (this.done(this.str), cheet.__done(this.str), this.reset())
          ));
    }),
    ...
    (cheet.__next = NOOP),
    (cheet.next = function(fn) {
      cheet.__next = null === fn ? NOOP : fn;
    }),
    (cheet.__fail = NOOP),
    (cheet.fail = function(fn) {
      cheet.__fail = null === fn ? NOOP : fn;
    }),
    (cheet.__done = NOOP),
    (cheet.done = function(fn) {
    ...
    (cheet.reset = function(id) {
      ...
      return seq instanceof Sequence
        ? void seq.reset()
        : void console.warn("cheet: Unknown sequence: " + id);
    }),
    (global.cheet = cheet),
...
window.cheet("up up down down left right left right b a", function() {
  window.location.href = "https://www.youtube.com/embed/NWD7iqtOJSE?autoplay=1";
});
